...
const ApprovalTrail: React.FC<ApprovalTrailProps> = ({ currentStage, activity, viewOnly }) => {
  ...
  const { data: activityAppeal } = useQuery({
      ...
      queryFn: async () => await activityAppealService.getAll({ personnelActivityId: activity?.personnelActivityId }),
      initialData: []
    });
  ...
  const mapping: Record<number, any> = {
    1: { isApprove: approvalData?.stageOneIsApprove, remarks: approvalData?.stageOneRemarks, approver: approvalData?.approverOne },
    2: { isApprove: approvalData?.stageTwoIsApprove, remarks: approvalData?.stageTwoRemarks, approver: approvalData?.approverTwo },
    3: { isApprove: approvalData?.stageThreeIsApprove, remarks: approvalData?.stageThreeRemarks, approver: approvalData?.approverThree },
    4: { isApprove: approvalData?.stageFourIsApprove, remarks: approvalData?.stageFourRemarks, approver: approvalData?.approverFour },
  };
  ...
  const renderStageDetail = (
    ...
    const statusConfig = isApproved
      ? { bg: "bg-emerald-50", border: "border-emerald-200", label: "Approved by:" }
      : { bg: "bg-rose-50", border: "border-rose-200", label: "Disapproved by:" };

    
    return (
      <div className={`${statusConfig.bg} ${statusConfig.border} p-3 rounded-lg border mt-2 text-[12px] shadow-sm`}>
  {/* Existing Header */}
  <div className="flex justify-between items-start mb-2">
    <span className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold">
      {statusConfig.label} <b className="text-[11px] text-gray-600">{nameFormat(personnel) || "System Administrator"}</b>
    </span>
    <Tag color={isApproved ? "success" : "error"} className="text-[10px] m-0 px-1 font-bold">
  ...
  const getStepStatus = (stageNumber: number, isApproved: boolean | null | undefined) => {
...
}, [workflowSteps, roles, personnelType, currentStage, approvalData, viewOnly]);
  ...
  return (
    <div className="pl-4 h-full">
      ...
      {dynamicItems.length > 0 ? (
        ...
        <Text type="secondary">No workflow steps configured for {personnelType}.</Text>
      )}
    </div>
  );
};
